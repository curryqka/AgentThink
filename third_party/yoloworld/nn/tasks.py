...
class WorldModel(DetectionModel):
    ...
    def set_classes(self, text, batch=80, cache_clip_model=True):
        ...
        # 指定本地模型路径
        model_path = '/high_perf_store/mlinfra-vepfs/qiankangan/Drive-MLLM-main/third_party/ckpt/ViT-B-32.pt'  # 根据实际路径修改
        
        if not getattr(self, "clip_model", None) and cache_clip_model:
            # 从本地加载模型
            # breakpoint()
            self.clip_model = clip.load("ViT-B/32", download_root=model_path.rsplit('/', 1)[0])[0]
            
        # 创建临时加载函数
        def load_local_clip():
            model, _ = clip.load("ViT-B/32", download_root=model_path.rsplit('/', 1)[0])
            return model
        
        model = self.clip_model if cache_clip_model else load_local_clip()
